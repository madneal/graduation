...
\centerline{\LARGE{\hei  论文题目:~~\underline{分布估计算法中基于差分进化的采样策略 }}}
...
\section*{\hspace*{\fill}\Huge {分布估计算法中基于差分进化的采样策略}\hspace*{\fill}}
...
本文研究的是对于演化算法中采样策略的相关工作。受差分进化思想所启发，在本文中提出了一种基于差分进化的采样策略，并将这种采样策略应用于分布估计算法之中，从而提高算法性能和运行效率。本文将基于差分进化的采样策略和分布估计算法相结合，并且分别研究其在解决多目标以及单目标问题上的性能表现。通过综合的实验结果分析，可以看出基于差分进化的采样策略在解决分布估计算法中的采样问题具有较好的表现，并且具有很大的潜力。
\\[2mm]
...
\large{2.1 多目标优化问题}$\dotfill 3$\par
\large{2.2 单目标优化问题}$\dotfill 4$\par
\large{2.3 分布估计算法}$\dotfill 4$\par
\large{2.4 差分进化}$\dotfill 10$\\[4mm]

\large{3 对于连续多目标优化问题的研究}\hfill \rmfamily{17}\par
\large{3.1 连续多目标优化问题}$\dotfill 17$\par
\large{3.2 基于差分进化的采样策略的多目标分布估计算法}$\dotfill 17$\par
\large{3.3 实验分析}$\dotfill 22$\par
\large{3.4 本章小结}$\dotfill 25$\\[4mm]

\large{4 对于全局单目标优化问题的研究}\hfill \rmfamily{28}\par
\large{4.1 全局单目标优化问题}$\dotfill 28$\par
\large{4.2 基于差分进化的采样策略的单目标分布估计算法}$\dotfill 28$\par

...
\vspace*{5mm} \normalfont
演化算法是一种受生物进化启发的基于种群的启发式的优化算法。演化算法已经被广泛应用于工程、生物学、经济学、基因工程以及社会科学等。
...
本文研究的是分布估计算法中的采样策略的相关工作。传统的分布估计算法通过使用建立概率模型，然后从建立的概率模型中采样。本文受差分进化思想所启发，提出了一种基于差分进化的采样策略，即差分采样策略，并将这种采样策略应用于分布估计算法之中，从而提高算法性能和运行效率。本文将差分采样策略和分布估计算法相结合，并且分别研究其在解决单目标和多目标优化问题上的性能表现。通过综合的实验结果分析，可以看出差分采样策略对于提高分布估计算法中的采样具有重大意义，并且具有很大的潜力。

本文的主要内容是基于差分采样策略的单目标分布估计算法和多目标分布估计算法。对于单目标优化问题，基于DE/EDA算法框架，通过结合基于特征向量的差分进化来采样，同时，利用expensive local search(LS) 来进一步提高解集质量，提出了基于差分采样的单目标分布估计算法。另一方面，对于多目标优化问题，通过差分采样策略来改进RM-MEDA的采样，并且效果显著。通过将差分采样策略分别应用单目标和多目标分布估计算法，并且在综合的实验分析的基础上，有效地表明了差分采样策略对于分布估计算法的重大意义。
\\[2mm]

\bfseries{ 关键词:}\normalfont\quad\ 分布估计算法，差分进化，演化算法，全局优化多目标优化
...
\vspace*{5mm} \normalfont
Evolutionary algorithm is a heuristic optimization algorithm based on population which is inspired by biological evolution. Evolution algorithms are applied to various problems, as it does not need any complex assumptions. Evolutionary algorithms have been widely applied to engineering, bibliography, economy, gene engineering and social science, etc.
...
Differential evolution has attracted many researchers since proposed in 1995. Differential evolution is a kind of simple but effective stochastic optimization algorithm. Due to the advantages of differential evolution, it has been widely utilized in different fields. There have been quantities of significant work of differential evolution on single objective optimization, multiobjective optimization and constrained optimization, etc.

This paper mainly studies the sampling strategy of estimation of distribution algorithm. Inspired by differential evolution, this paper proposes a sampling strategy based on differential evolution, namely differential evolution sampling strategy. And this strategy is utilized in estimation of distribution algorithm to study its performance. This paper combines differential evolution sampling strategy with estimation of distribution algorithm, and study its performance on single objective problems and multiobjective problems respectively. According to the comprehensive experimental analysis, it can be seen that differential evolution sampling strategy is impressively significant to improve the sampling of estimation of distribution algorithm.

The main contents of this paper are sing objective estimation of distribution algorithm and multiobjective estimation of distribution algorithm based on differential evolution sampling strategy. For single objective optimization problems, based on the framework of DE/EDA, utilize a differential evolution based on eigenvector to sample. Meanwhile, expensive local search is applied to improve the quality of solutions. Hence, single objective estimation of distribution algorithm is proposed. On the other hand, for multiobjective optimization problems, utilize differential evolution sampling strategy to improve the sampling of RM-MEDA and obtain positive results. This paper applies differential evolution sampling strategy to single objective estimation of distribution algorithm and multiobjective estimation of distribution algorithm. The comprehensive and systematic experimental analysis indicates the significance of differential evolution sampling strategy in estimation of distribution algorithm.

\vspace*{3mm}

\bfseries{Key words:}\normalfont \quad ~estimation of distribution algorithm,~differential evolution,~evolutionary algorithm,~global optimization
...
差分进化是一种有效并且简单的演化算法~\cite{DE1,DE2}。差分进化通过利用种群中的距离和方向信息来指导搜索的过程。差分进化通过提取种群中的差分信息来指导下一步搜素并且可以加快解集收敛的速度。得益于这一点，受到差分进化算法思想的启发，我们提出基于差分进化的采样策略，即差分采样策略（Differential Evolution based on Sampling, DES)~\cite{des}。通过利用分布估计算法中概率模型不断收集种群中的统计信息，不断地更新概率模型，指导搜索的进行，同时利用差分采样策略产生精英个体，提高种群的多样性，在保证种群多样性的同时，也加速了最终结果的收敛，从而进一步提高算法的性能。

\subsection{本文所做的工作和内容安排}
针对分布估计算法的采样，提出差分采样策略，用于提高分布估计算法的性能。针对单目标和多目标优化问题，通过一系列的综合实验分析来研究差分采样策略对于提高分布估计算法性能的意义。在单目标优化问题上，基于差分进化和分布估计算法的混合算法（DE/DEA)~\cite{DEEDA2005}，通过改进DE/EDA 中的差分进化算法，并且同时提出一种基于特征向量的改进的差分进化和分布估计算法的混合算法（EDA/DE-EIG），研究其对于提高分布估计算法对于解决单目标优化问题上算法性能和速度提升的意义。在多目标优化问题上，DES被用于改进基于规律模型的分布估计算法（A Regularity Model-based Estimation of Distribution Algorithm, RM-MEDA)~\cite{RM-MEDA} 中的采样策略，提高算法性能。
...
第四章 ~~~对于连续多目标优化问题的研究，在RM-MEDA的基础上提出差分采样策略，通过这种新型的采样策略去改进RM-MEDA的采样环节。即在采样环节中，将种群转换到隐空间中，采取一种新型的变异策略对种群进行变异操作，然后再将个体转换到原始的空间中，从而产生新的个体。具体描述了相关的研究背景和算法框架和具体实现。最后，通过综合的实验分析了RM-MEDA中存在的问题，并且通过一系列综合实验分析来分析差分采样策略的重要意义。
...
演化算法是另外一种适用于求解最优化问题的方法。演化算法主要包括：遗传算法、遗传编程、演化编程、演化策略、分布估计算法、差分进化以及其他的群体智能优化算法。演化算法被广泛地应用于单目标最优化问题。然而现实世界中的问题往往涉及多个目标，因此多目标演化算法（Multiobjective Evolutionary Algorithms, MOEAs）的研究意义也极为重大。现在对于多目标演化算法的研究百花齐放，可以将多目标演化算法分为以下几类：
\begin{itemize}
...
其中$X_{r_1}$, $X_{r_2}$, $X_{r_3}$是种群中的随机个体，$r_1, r_2, r_3$则是从1到NP(NP是种群的大小)之间选择的三个互不相同的整数。F是变异策略中的缩放因子，rand是0到1之间服从均匀分布的随机数。

...
本文的主要内容是基于差分采样策略的单目标分布估计算法和多目标分布估计算法。对于单目标优化问题，基于DE/EDA算法框架，通过结合基于特征向量的差分进化来采样，同时，利用expensive local search(LS) 来进一步提高解集质量，提出了基于差分采样的单目标分布估计算法。另一方面，对于多目标优化问题，通过差分采样策略来改进RM-MEDA的采样，并且效果显著。通过将差分采样策略分别应用单目标和多目标分布估计算法，并且在综合的实验分析的基础上，有效地表明了差分采样策略对于分布估计算法中的重大意义。
\\[2mm]
...
\subsection{研究目的和意义}
分布估计算法是一种新兴的演化算法，它通过建立一个概率模型，然后通过在这个概率模型中采样来产生新的种群个体，通过提取当前精英中的统计信息来进一步地更新概率模型来指导下一步的搜索~\cite{eda2002}~\cite{edasurvey2002}。不同于传统的演化算法，在分布估计算法中不存在交叉和变异，而是通过使用一个精确的概率模型来提取种群的统计信息。子代种群个体将会通过从建立的概率模型中采样产生，并且产生的个体会部分或者全部地取代父代中区个体。分布估计算法通过概率模型能够更好地进行全局搜索，提取全局统计信息。分布估计算法主要由三个步骤组成，即：建模，采样，选择。采样是分布估计算法的重要环节，好的采样有利于精英个体的产生。本文主要针对分布估计算法中的采样，提升采样的性能可以大大地提高算法的性能，更有可能产生更优的精英个体~\cite{des}。

差分进化是一种有效并且简单的演化算法~\cite{DE1,DE2}。差分进化通过利用种群中的距离和方向信息来指导搜索的过程。差分进化通过提取种群中的差分信息来指导下一步搜素并且可以加快解集收敛的速度。得益于这一点，受到差分进化算法思想的启发，我们提出基于差分进化的采样策略，即差分采样策略（Differential Evolution based Sampling, DES)~\cite{des}。通过利用分布估计算法中概率模型不断收集种群中的统计信息，不断地更新概率模型，指导搜索的进行，同时利用差分采样策略产生精英个体，提高种群的多样性，在保证种群多样性的同时，也加速了最终结果的收敛，从而进一步提高算法的性能。

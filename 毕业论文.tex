...
\end{center}
  ...
  \centerline{\sanhao\textbf{ Title:~~\underline{The study of the sampling strategy based on differential} }}
  \vspace{0.15cm}
\centerline{\sanhao\textbf{ ~~~~~~~~~~\underline{evolution in estimation of distribution algorithms } }}
...
差分进化自从1995年被提出之后就受到研究者的广泛关注。差分进化是一种简单但却有效的随机优化算法，得益于它的诸多优点，它已经被广泛应用于各个领域。差分进化在单目标全局优化，多目标优化，约束优化等领域受到了广大学者的研究和关注。

本文研究的是分布估计算法中的采样策略的相关工作。受差分进化思想所启发，在本文中提出了一种基于差分进化的采样策略，即差分采样策略，并将这种采样策略应用于分布估计算法之中，从而提高算法性能和运行效率。本文将差分采样策略和分布估计算法相结合，并且分别研究其在解决单目标和多目标优化问题上的性能表现。通过综合的实验结果分析，可以看出差分采样策略对于提高分布估计算法中的采样具有重大意义，并且具有很大的潜力。

本文的主要内容是基于差分采样的单目标分布估计算法和多目标分布估计算法。对于单目标优化问题，基于DE/EDA算法框架，通过结合基于特征向量的差分进化来采样，同时，利用expensive local search(LS) 来进一步提高解集质量，提出了基于差分采样的分布估计算法。另一方面，对于多目标优化问题，通过差分采样策略来改进RM-MEDA的采样，并且效果显著。通过将差分采样策略分别应用单目标和多目标分布估计算法，并且在综合的实验分析的基础上，有效地表明了差分采样策略对于分布估计算法中的重大意义。
\\[2mm]
...
\thispagestyle{plain}
\section*{\hspace*{\fill}\Large{Differential Evolution Sampling Strategy in}\hspace*{\fill}
...
{\large 3 基于差分采样的单目标优化}\hfill \rmfamily{17}\par
...
第四章 ~~~对于全局单目标优化问题，为了进一步提高单目标分布估计算法的采样性能，利用差分采样策略来进一步改进单目标分布股计算法的采样性能。基于DE/EDA算法框架，引入基于特征向量的差分进化算法，并且通过结合expensive LS来进一步提高算法性能，最终提出基于差分采样策略的单目标分布估计算法EDA/DE-EIG。 通过和JADE，DE/EDA的综合实验分析对比，分析出基于差分进化的采样策略在解决单目标优化问题的作用和意义。

第三章 ~~~对于连续多目标优化问题的研究，在RM-MEDA的基础上提出差分采样策略，通过这种新型的采样策略去改进RM-MEDA的采样环节。即在采样环节中，将种群转换到隐空间中，采取一种新型的变异策略对种群进行变异操作，然后再将个体转换到原始的空间中，从而产生新的个体。具体描述了相关的研究背景和算法框架和具体实现。最后，通过综合的实验分析了RM-MEDA中存在的问题，并且通过一系列综合实验分析来分析差分采样策略的重要意义。
...
\textbf{基于多目标的分布估计算法：}上述介绍的分布估计算法基本上是解决单目标优化问题的。然而，现实世界中的问题往往是具有多个目标的。一种针对多目标优化问题的解决方案是根据目标权重将多目标优化问题转化为单目标优化问题。但是，更好的方式在目标之间找到一个最好的权衡从而构建一个帕累托最优~\cite{mop2001}。 简而言之，帕累托最优要比在任意目标上的最优解要更好。因此，分布估计算法被用于在多目标优化问题中寻找多种帕累托最优解。

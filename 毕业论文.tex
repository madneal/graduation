...
分布估计算法自从1994年首次提出后，就得到了广泛地应用于解决各种大型的复杂问题，包括军事天线设计~\cite{military2007}，多目标背包问题~\cite{knapsack2011}，地下水治理~\cite{groundwater2002}，森林管理~\cite{forest2005}等。 值得强调的一点是，在解决相同等级大小和复杂度的问题的时候，分布估计算法往往就是最优的算法。

根据分布估计算法针对的不同问题，可以将分布估计算法分为以下几种类型：

基于离散变量的分布估计算法：对于分布估计算法，模型的种类是比较重要的环节，在此，可以分为，单变量模型，基于树的模型以及以及多元关系模型。

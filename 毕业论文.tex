...
建模是分布估计算法中关键环节，在RM-MEDA中，如何找到种群中的个体在决策空间中的中心曲线或曲面是建模的关键。在此假设$\xi$由$k$个流体$M^1,\cdots,M^k$ 组成，每一个流体是一个超矩形。一般来说，对于双目标，每一个流体$M^j$是一个线段，对于三个目标，每一个流体$M^j$是一个二维矩形。建模的重点就是估计流体$M^j$以及流体的主要组成部分$\zeta$。为了更好地描述这些流体，通过LPCA将种群划分为$k$个聚类：$C_1,\cdots,\C_k$。 令$S$为$R^n$ 的有限子集，则定义子集$S$的均值为：
\begin{equation}
\label{s}
\overline{x}=\frac{1}{|S|}\sum_{x\in{S}}x
\end{equation}
...
\begin{equation}
\label{cov}
Cov=\frac{1}{|S|-1}\sum_{x\in{S}}(x-\overline{x})(x-\overline{x})^T
\end{equation}

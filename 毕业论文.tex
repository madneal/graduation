...
针对分布估计算法的采样，提出差分采样策略，用于提高分布估计算法的性能。针对单目标和多目标优化问题，通过一系列的综合实验分析来研究差分采样策略对于提高分布估计算法性能的意义。在单目标优化问题上，基于差分进化和分布估计算法的混合算法（DE/DEA)~\cite{DEEDA2005}，通过改进DE/EDA 中的差分进化算法，并且同时提出一种基于特征向量的改进的差分进化和分布估计算法的混合算法（EDA/DE-EIG），研究其对于提高分布估计算法对于解决单目标优化问题上算法性能和速度提升的意义。p在多目标优化问题上，DES被用于改进基于平滑模型的分布估计算法（A Regularity Model-based Estimation of Distribution Algorithm, RM-MEDA)~\cite{RM-MEDA} 中的采样策略，提高算法性能。
...
~\ref{4.1}节简要地介绍了全局单目标优化问题；~\ref{4.2}节主要介绍了基于差分采样策略单目标分估计算法的算法框架和具体实现；~\ref{4.3}节主要介绍了实验设置以及需要比较的算法，通过综合的实验分析比较，最终得出对于基于差分进化的采样的采样策略算法性能的综合评价；~\ref{4.4}节做出了对本章的小节。

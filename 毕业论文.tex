...
第四章 ~~~对于全局单目标优化问题的研究，基于DE/EDA算法框架，引入基于特征向量的差分进化算法，并且通过结合expensive LS来进一步提高算法性能，最终提出基于差分采样策略的单目标分布估计算法EDA/DE-EIG。通过和JADE，DE/EDA的综合实验分析对比，分析出基于差分进化的采样策略在解决单目标优化问题的作用和意义。

第三章 ~~~对于连续多目标优化问题的研究，提出差分采样策略，通过这种新型的采样策略去改进RM-MEDA的采样环节，具体描述了相关的研究背景和算法框架和具体实现。最后，通过综合的实验分析了RM-MEDA中存在的问题，并且通过一系列综合实验分析来分析差分采样策略的重要意义。

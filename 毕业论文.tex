...
{\large 致\quad 谢}$\dotfill 50$\par
{\large 在校期间的学术论文和科研项目$\dotfill 51$}\par
...
差分进化是一种有效并且简单的演化算法~\cite{DE1,DE2}。差分进化通过利用种群中的距离和方向信息来指导搜索的过程。差分进化通过提取种群中的差分信息来指导下一步搜素并且可以加快解集收敛的速度。分布估计算法中传统的采样一般是根据建立的概率模型进行采样。因此，如何建立并且更新概率模型是一个比较困难的问题。基于以上问题，并且受到差分进化算法思想的启发，我们提出基于差分进化的采样策略，即差分采样策略（Differential Evolution based on Sampling, DES)~\cite{des}。 通过利用分布估计算法中概率模型不断收集种群中的统计信息，不断地更新概率模型，指导搜索的进行，同时利用差分采样策略产生精英个体，提高种群的多样性，在保证种群多样性的同时，也加速了最终结果的收敛，从而进一步提高算法的性能。
...
第三章 ~~~对于全局单目标优化问题，为了进一步提高单目标分布估计算法的采样性能，利用差分采样策略来进一步改进单目标分布股计算法的采样性能。基于DE/EDA算法框架，引入基于特征向量的差分进化算法，并且通过结合expensive LS来进一步提高算法性能，最终提出基于差分采样策略的单目标分布估计算法EDA/DE-EIG。 通过和JADE，DE/EDA的综合实验分析对比，分析出基于差分进化的采样策略在解决单目标优化问题的作用和意义。

第四章 ~~~对于连续多目标优化问题的研究，在RM-MEDA的基础上提出差分采样策略，通过这种新型的采样策略去改进RM-MEDA的采样环节。即在采样环节中，将种群转换到隐空间中，采取一种新型的变异策略对种群进行变异操作，然后再将个体转换到原始的空间中，从而产生新的个体。具体描述了相关的研究背景和算法框架和具体实现。最后，通过综合的实验分析了RM-MEDA 中存在的问题，并且通过一系列综合实验分析来分析差分采样策略的重要意义。

第五章 ~~~对本文进行全面地总结，并且根据目前工作的不足之处提出未来的展望。

\newpage
\section{研究背景}
本章首先介绍了最优化问题，以及最优化问题在数学领域中的定义。对于最优化问题的求解，介绍了传统的优化方法和演化算法。针对本文的重点讨论的单目标和多目标优化问题，简要地介绍了单目标和多目标演化算法，并将多目标演化算法分为四大类。接着着重介绍了分布估计算法的算法框架和研究现状。最后介绍了差分进化的算法框架和研究现状。
...
\item 混合多目标演化算法：在多目标演化算法中，每一种技术都具有各自的特点和优点。将这些技术的特色和优点结合起来从而葱郁解决复杂的多目标优化问题。
\end{itemize}

演化算法被广泛地应用于解决最优化问题，本文的重点是利用分布估计算法来求解单目标和多目标优化问题。分布估计算法是一种新型的用于解决最优化问题的算法。差分进化是另外一种得到广泛研究的演化算法。下面主要讲解分布估计算法和差分进化的算法框架和研究现状。
...
对于单变量模型的例子，比如单变量边缘分布算法（UMDA）~\cite{umda1996},用于求解onemax 问题。然而大多数的分布估计算法是通过维护一个候选解种群来工作的。增量分布估计算法通过概率模型来替换种群。这个概率模型开始在解空间中服从均匀分布，在之后的每一个迭代之中，都会产生新的解，最好的解或者相对来说比较好的解会被选择进入下一代迭代。这个概率模型就会根据解集的性质慢慢发生变化。通过这种方式，概率模型不断地获得改良，并且不需要将大量的种群全部存储起来。基于种群的增量学习（PBIL）~\cite{pbil1994}是一种作用于二进制串的单变量的增量分布估计算法。像UMDA 一样，PBIL 也是通过概率模型来构建概率向量。另外，为了防止种群过早收敛，会通过一个变异比率参数在每一次迭代中对概率向量进行微调。压缩遗传算法（cGA）~\cite{cga1999}是另一种增量的单变量增量分布估计算法。和PBIL类似，cGA也是通过使用概率向量来代表整个种群中的解集，使用指定长度的二进制串进行编码。cGA和PBIL最主要的区别就是这两者在每一次迭代中更新概率向量的方法。

单变量模型是基于变量之间是相互独立的，然而很多问题的变量之间往往是相互联系的。双变量依赖假设是比变量独立假设更为宽松的假设，假设随机变量的依赖性仅存在于两个变量之间。树形表示模型是一种常用的用于分布估计算法的概率模型。MIMIC~\cite{mimic1997} 是一种通过链分布来描述变量之间关系的分布估计算法。假设问题中n 个变量的排列为，$\pi=i_1, i_2\cdots{i_n}$，MIMIC可以将$p(X_1, X_2, \cdots, X_n)$的概率分布分解为：
\begin{equation}
\label{mimic}
...
\textbf{多目标分布估计算法：}上述介绍的分布估计算法基本上是解决单目标优化问题的。然而，现实世界中的问题往往是具有多个目标的。一种针对多目标优化问题的解决方案是根据目标权重将多目标优化问题转化为单目标优化问题。但是，更好的方式在目标之间找到一个最好的权衡从而构建一个帕累托最优~\cite{mop2001}。 简而言之，帕累托最优要比在任意目标上的最优解要更好。因此，分布估计算法被用于在多目标优化问题中寻找多种帕累托最优解。

对于多目标优化问题，就不可能找到一个解可以使所有的目标达到最优解。最终的目标是找到帕累托最优解集的宽分布。贝叶斯网络多目标优化算法（BMOA）~\cite{bmoa2002}使用一个特别的选择选择算子$\epsilon$-archive来更新帕累托最优解集并且保证种群的多样性。这个选择算子会维护一个最小的解集，这个算子会支配其他产生的解。本质上来说，BMOA是将混合BOA~\cite{mboa2004}和SPEA2~\cite{spea2-2002}结合起来。
...
在传统意义中的演化计算中，变异是通过一个随机元素从而使种群发生改变或者扰动从而产生变异向量来找到更好的解。在差分进化中，对于每一个目标向量$x_{i,G}$都会使用某一种变异策略来产生相应的变异向量$v_{i,G}$。下面是六种文献中常用的变异策略：
...
Price在2006年又提出一种先进的DE/rand/1-either-or算法~\cite{de2006}。在这个算法中，在概率$p_F$下，试验向量全都是突变体，而在概率$1-p_F$下，试验向量全都是重组体。这一新的方法相对于传统的rand/1/bin和target-to-best/1/bin有着较大的竞争力。试验向量的产生方式如下：
...
大多数的演化算法的运行效率取决于它们在搜索时勘探和开采的趋势。为了更好地平衡演化算法中的勘探和开采的趋势，Das提出了两种邻域拓扑模型~\cite{nde2009}。根据这一原理，提出了基于全局和局部邻域的差分进化（DEGL），对于DE/target-to-best/1策略做出了重大改进。
...
{\bf 针对多目标优化问题的差分进化算法：}得益于差分进化的简单但却强大的特点，差分进化同时也得到了多目标研究学者的广泛关注。 Chang等人第一次将差分进化拓展用于解决多目标优化问题~\cite{demo1999}。Abbas和Sarkar提出了帕累托差分进化算法（PDE）通过连续变量来解决多目标优化问题~\cite{pde2002}，并且这个算法在和其它多目标优化算法相比具有很大的竞争力。Kukkonen 和Lampinen通过将DE/rand/1/bin进行拓展用于解决多目标优化问题，提出广义差分进化（GDE）~\cite{gde2004}。Xue等人提出了一个多目标差分进化~\cite{mode2005} （MODE），利用最好的个体来产生后代。通过基于Pareto的方法来挑选最好的个体。同时，还使用了$(\mu+\lambda)$选择算子，Pareto 分级以及聚集距离来产生新解并维持其良好的分布。Robic和Filipic提出了另外一种对于多目标优化问题的差分进化DEMO~\cite{demo2005}，这个算法结合差分进化的优点以及基于Pareto 分级和聚集距离排序的机制。DEMO仅维持一个种群并且当产生新解的时候这个种群会被进行拓展。这能够确保种群向Pareto front 快速收敛，然而非劣排序和聚集距离的使用造成了解集的均匀扩撒。Iorio等人提出非劣排序差分进化算法（NSDE）~\cite{nsde2004}，这个算法是基于NSGA-II 做出了一些改动。这两个算法的主要区别时产生新的个体的方式。NSGA-II使用实数编码交叉算子和变异算子，而NSDE则是使用差分进化中的交叉算子和变异算子。NSDE在解决旋转的多目标优化问题上要优于NSGA-II。
...
~\ref{3.1}节简要地介绍了全局单目标优化问题；~\ref{3.2}节主要介绍了基于差分采样策略单目标分估计算法的算法框架和具体实现；~\ref{3.3}节主要介绍了实验设置以及需要比较的算法，通过综合的实验分析比较，最终得出对于基于差分进化的采样的采样策略算法性能的综合评价；~\ref{3.4}节做出了对本章的小节。

\subsection{全局单目标优化问题}
\label{3.1}
%全局单目标优化问题是应用数学和数理分析的一个分支。它通过某些标准来求解一个或者多个函数的全局最优解。

单目标优化问题几乎出现在科学和工程应用的各个领域中。单目标优化问题的目标函数一般是非凸函数，并且在可行区域内具有很多的局部极小值或者极大值。本文研究的单目标优化问题针对的是连续空间的全局优化问题，一般即是求目标函数的最小值或者最大值。全局优化问题是应用数学和数值分析的一个分支，用于解决在一定条件下求得一个或者一组函数的最优解~\cite{global2000}。一般来说，对这些函数会有一系列的限制，并且这些决策变量会会依据这些限制做优化。全局优化问题和普通优化问题的区别是其是在所有的输入值中寻找最大值或者最小值，而不是寻找局部的最大值或者最小值。
...
差分进化是一种非常成功的用于解决全局连续问题优化的算法。它主要是利用当前种群中的距离和方向信息来指导未来的搜索。分布估计算法则是通过从建立的概率模型中采样来产生新的种群个体。DE/EDA 就是将差分进化和分布估计算法结合起来，用于解决全局连读优化问题优化。通过利用分布估计算法可以提取种群全局信息和差分进化可以提取种群差分信息的优点，DE/EDA 是一种非常具有研究前景的算法。本章节将会基于DE/EDA这一算法框架作进一步的研究。
...
基于DE/EDA框架，将DE-EIG算法引入到之中，来进一步改善分布估计算法的采样环节。同时为了保持种群的多样性，通过一个随机参数来控制种群是通过概率模型采样产生亦或是通过DE-EIG来产生。这样，可以增加种群的多样性，同时也可以提高差分进化的搜索能力。同时，expensive LS也将会被用于进一步提升算法求解的性能。
...
\begin{enumerate}
  \item 测试题中所有种群维度都设置为30.所有算法都会在每一个测试题上独立运行50 次，体积条件是450000函数评估。
...
同时，为了从一个更加客观的角度来比较最终的结果，威尔克松统计分析被用来比较这几个算法。对于JADE，EDA/DE-EIG在3个测试题上要优于JADE；这两个算法在6个测试题上取得了近似的结果；在4 个测试题上，JADE的最终结果更好。在与DE/EDA相比，EDA/DE-EIG有着不小的提升。同时，对于JADE，这两个算法从统计上来说在13个测试题上有着近似的表现。这表明DE-EIG的确取得了有意义的效果。这证明EDA/DE-EIG的性能表现还是具有潜力的同时也需要进一步的提升研究。
...
\subsection{本章小结}
\label{3.4}
DE/EDA是一种对于全局优化的具有很大潜力的方法，它结合了分布估计算法的全局建模信息和差分进化的局部搜索信息。在本章节中，通过引进一种基于特征向量的差分进化算法，DE-EIG。DE-EIG有利于更好地利用种群的统计信息并且加速收敛。同时，通过结合expensive LS来进一步提炼演化的解集，并且通过随机参数的设置，从而保证种群的多样性。综上，我们提出了一种基于差分采样的单目标分布估计算法。实验结果表明，在和JADE 和DE/EDA 比较的过程中，单目标分布估计算法还是比较具有一定的优势，在解决单目标全局优化问题具有一定的竞争力。
...
~\ref{4.1}节给出了连续多目标问题的相关背景知识以及定义；~\ref{4.2}节主要介绍差分采样策略以及基于差分采样策略的多目标分布估计算法的算法框架；~\ref{4.3}节通过实验分析了RM-MEDA 中存在的问题，并且综合展现了基于差分采样策略的多目标分布估计算法的性能表现，同时分析了参数$F$的敏感性；~\ref{4.4}对本章进行了总结。
...
多目标问题在很多工程领域都普遍存在，通常来说，多目标问题中的各个目标之间往往都是相互冲突的。在一般条件下，根据Karush-Kuhn-Tucker 可以推导出：连续多目标问题在决策空间中的Pareto set 是一个连续分段的（m-1）维的流形体（m是目标数）。对于一个成功的多目标演化算法（multiobjective evolutionary algorithm, MOEA）来说，独立的个体应该是在决策空间中分散在Pareto set 附近，如图~\ref{FIG1}所示。
\begin{figure}[ht]
...
建模是分布估计算法中关键环节，在RM-MEDA中，如何找到种群中的个体在决策空间中的中心曲线或曲面是建模的关键。在此假设$\xi$由$k$个流体$M^1,\cdots,M^k$ 组成，每一个流体是一个超矩形。一般来说，对于双目标，每一个流体$M^j$是一个线段，对于三个目标，每一个流体$M^j$是一个二维矩形。建模的重点就是估计流体$M^j$以及流体的主要组成部分$\zeta$。为了更好地描述这些流体，通过LPCA 将种群划分为$k$个聚类：$C_1,\cdots, C_k$。 令$C$为$R^n$ 的有限子集，则定义子集$C$ 的均值为：
\begin{equation}
\label{C}
...
\begin{itemize}
  ...
  \item \emph{种群大小}：对于测试题（$F3$, $F7$, $F9$）每一代的种群大小设置成100，对于其它的测试题，每一代种群大小设置为200.
  ...
  \item\emph{迭代次数}：对于测试题（$F1$, $F2$, $F5$, $F6$）迭代次数设置为100，对于测试题$F4$和$F8$设置为200，其它的设置为1000。
\end{itemize}
...
\begin{itemize}
\item 将差分进化采样策略应用于其他的多目标演化算法，比如基于分解的多目标演化算法，从而进一步研究基于差分进化采样策略的潜力和价值。
